...
public:
    ...
    NativeAccessor(ContextType ctx, std::shared_ptr<Realm> realm=nullptr)
    : m_ctx(ctx), m_realm(std::move(realm)) { }

    NativeAccessor(NativeAccessor& parent, const Property& prop)
    : m_realm(parent.m_realm), m_object_schema(&*m_realm->schema().find(prop.object_type))
    { }
    ...
    OptionalValue default_value_for_property(Realm *realm, const ObjectSchema &object_schema, const std::string &prop_name) {
        auto defaults = get_delegate<JSEngine>(realm)->m_defaults[object_schema.name];
